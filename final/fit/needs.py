...
# 화면에 쓰는 쉬운 말. 「인력 수요」 같은 행정 용어를 그대로 내보내지 않는다.
#
# 이름만 떼어 놓아도 무엇인지 알아야 한다. 배지·막대 같은 좁은 자리에는 이 이름만
# 나가고 설명이 따라붙지 못하기 때문이다. 「사람」·「돈」·「기술」은 그 자리에서
# 무슨 사람인지 무슨 돈인지 알 수 없어 바꿨다.
PLAIN = {
    "인력": "일할 사람",
    "기술": "기술 개발",
    "금융": "기업 자금",
...
# 이름 밑에 다는 한 줄. 무엇을 세는 칸인지 예를 들어 못박는다.
#
# 금융을 「예산」이라 부르지 않는 이유가 있다. 예산 화면이 말하는 예산은 **이 사업에
# 붙은 돈**이고, 여기 금융은 **기업이 밖에서 구하는 돈**이다. 두 말이 같은 이름을
# 쓰면 「예산 30억이 있는데 왜 돈이 비었나」로 읽힌다.
HINT = {
    "인력": "뽑을 사람 · 기술 가진 사람",
    "기술": "연구개발 · 기술이전 · 시험분석",
    # 괄호로 감싸지 않는다 — 검토서가 이 줄을 다시 괄호에 넣어 괄호가 겹친다.
    "금융": "융자 · 보증 · 투자처럼 기업이 구하는 돈 — 이 사업에 붙은 예산과 다릅니다",
    "판로": "판매처 · 수출 · 구매처",
    "공급망": "부품·소재를 대 줄 협력기업",
    "시설": "입주공간 · 산업단지 · 공동장비",
    "행정": "인허가 · 규제 · 집행 절차",
...
def plain(need):
    return PLAIN.get(need, need)


def hint(need):
    return HINT.get(need, "")
