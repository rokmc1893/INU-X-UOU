...
def draft(card, findings, coverage, track_name, today, name_of=lambda p: p):
    ...
    lines = [
        ...
        "## 2. 판정 결과 (전부 후보)",
        "",
        f"### 2-1. 조정이 필요한 중복 후보 — {len(oh)}건",
        _bullet([pair(f) for f in oh]),
        "",
        f"### 2-2. 중복이 아닌 것 (의도적 병행 {len(oi)}건 / 보완 관계 {len(cp)}건)",
        "",
