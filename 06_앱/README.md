...
인천시 청년 일자리 정책 사이의 **지원 공백 · 인계 공백 · 조정 필요 중복 · 의도적 병행**을 규칙으로 판정해, 공무원의 「유사·중복 사업 자체 검토서」 초안을 만들어 주는 Streamlit 앱.
...
| 명령 | 결과 |
|---|---|
| `python -m pytest tests -v` | 테스트 27건 |
...
engine/
  ...
  detect.py         규칙 4종 + 동일 논리 Cypher. 판정에 LLM 불개입
...
| `data/demand/` | 수요신호 4건 | **1건만 실신호(B/D급), 3건은 가상 표본.** 화면에 그대로 표기 |
| `data/gold/` | 자작 정답셋 36건 | **자기 채점이며 2인 교차판정 전 잠정치.** 독립 판정자 2차 대조는 `judge2_pass.csv` |
